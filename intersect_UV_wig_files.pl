...
if ( $numargs != 2 )
{
	die "You need to pass 2 wig filenames as arguments; usage: perl add_UV_wig_files.pl <initial 0 wigfile1> <wigfile2>\n";
}
...
for ( my $i = 0; $i < scalar @ARGV; $i++ )
{
	...
	while ( my $line = <WIG> )
	{
			...
			{
				if ( defined $reads{$chr}{$field[0]} && $reads{$chr}{$field[0]} == 0) 
				{
					$reads{$chr}{$field[0]} = $field[1];
				}
				else 
